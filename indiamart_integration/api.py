...
def sync_india_mart_lead(from_date=None, to_date=None, **kwargs):
    frappe.log_error("Starting IndiaMART lead sync", "IndiaMART Sync Debug")
    print("Starting IndiaMART lead sync")
    try:
        # Fetch IndiaMart settings
        india_mart_setting = frappe.get_doc("IndiaMart Setting", "IndiaMart Setting")
        if not india_mart_setting.url or not india_mart_setting.key:
        ...
        lead_data = response.json()
        frappe.log_error(f"IndiaMART API Response: {json.dumps(lead_data, indent=2)}", "IndiaMART Sync Debug")
        print(f"API Response: {json.dumps(lead_data, indent=2)}")

        if not lead_data or "RESPONSE" not in lead_data:
            frappe.log_error("No leads found in IndiaMART response", "IndiaMART Sync Warning")
            print("No new leads found from IndiaMART")
            frappe.msgprint(_("No new leads found from IndiaMART for the specified date range."))
            return
        ...
        # Process each lead with original QUERY_TIME
        synced_lead_count = 0
        for lead in leads:
            frappe.log_error(f"Processing lead: {json.dumps(lead, indent=2)}", "IndiaMART Sync Debug")
            # Ensure QUERY_TIME is preserved when calling add_lead
            new_lead = add_lead(lead)
            if new_lead:
                synced_lead_count += 1
                frappe.log_error(f"Successfully synced lead: {new_lead.name} with creation {lead.get('QUERY_TIME')}", "IndiaMART Sync Success")
                print(f"Synced lead: {new_lead.name}")
            else:
                frappe.log_error(f"Failed to sync lead: {json.dumps(lead, indent=2)}", "IndiaMART Sync Warning")

        print(f"Successfully synced {synced_lead_count} leads from IndiaMART")
        frappe.msgprint(_(f"Successfully synced {synced_lead_count} leads from IndiaMART."))
...
def add_lead(lead_data):
    ...
    try:
        ...
        # Parse query time with validation
        query_time = None
        query_time_str = lead_data.get("QUERY_TIME")
        if query_time_str:
            try:
                query_time = datetime.strptime(query_time_str, "%Y-%m-%d %H:%M:%S")
                frappe.log_error(f"Successfully parsed QUERY_TIME: {query_time} for UNIQUE_QUERY_ID: {unique_query_id}", "IndiaMART Lead Creation Debug")
            except ValueError as e:
                frappe.log_error(f"Invalid QUERY_TIME format: {query_time_str} for UNIQUE_QUERY_ID: {unique_query_id}. Error: {str(e)}", "IndiaMART Lead Creation Warning")
                query_time = now_datetime()  # Fallback to current time only if parsing fails
        else:
            frappe.log_error(f"QUERY_TIME not found in lead data for UNIQUE_QUERY_ID: {unique_query_id}. Using now_datetime()", "IndiaMART Lead Creation Warning")
            query_time = now_datetime()
        ...
        doc = frappe.get_doc({
            ...
            "country": country_name,
            "status": lead_status,
            "creation": query_time,  # Explicitly set creation to query_time
            "modified": now_datetime(),
            "owner": frappe.session.user,
            "modified_by": frappe.session.user,
            "docstatus": 0,
            ...
            "custom_reciever_mobile": lead_data.get("RECEIVER_MOBILE"),
            "custom_reciever_catalog": lead_data.get("RECEIVER_CATALOG", ""),
            "custom_query_time": query_time_str  # Store original QUERY_TIME as a fallback
        }).insert(ignore_permissions=True)
    ...
    try:
        ...
        leads = frappe.get_all("Lead", 
            ...
            fields=[
                ...
                "custom_query_product_name", "custom_sender_address", "status", 
                "creation", "mobile_no", "email_id", "custom_query_message", "city",
                "custom_query_type", "custom_query_time"
            ]
        )
        
        frappe.log_error(f"Filtered leads for query_type '{query_type}': {len(leads)} leads", "IndiaMART Leads Fetch Debug")
        for lead in leads:
            frappe.log_error(f"Lead: {lead['name']}, Query Type: {lead['custom_query_type']}, Query Time: {lead['custom_query_time']}", "IndiaMART Leads Fetch Debug Detail")
...
@frappe.whitelist()
def delete_lead_by_query_id(query_id):
    """Delete a Lead by its custom_unique_query_id."""
    try:
        lead = frappe.get_all("Lead", filters={"custom_unique_query_id": query_id}, fields=["name"], limit=1)
        if not lead:
            frappe.msgprint(_(f"No Lead found with UNIQUE_QUERY_ID: {query_id}"))
            return {"success": False, "message": f"No Lead found with UNIQUE_QUERY_ID: {query_id}"}

        lead_name = lead[0].name
        frappe.delete_doc("Lead", lead_name, ignore_permissions=False, force=False)
        frappe.db.commit()
        frappe.log_error(f"Successfully deleted Lead with UNIQUE_QUERY_ID: {query_id} (Lead Name: {lead_name})", "IndiaMART Lead Deletion")
        frappe.msgprint(_(f"Successfully deleted Lead with UNIQUE_QUERY_ID: {query_id}"))
        return {"success": True, "message": f"Lead {lead_name} deleted successfully"}

    except Exception as e:
        frappe.log_error(f"Error deleting Lead with UNIQUE_QUERY_ID: {query_id}\n{frappe.get_traceback()}", "IndiaMART Lead Deletion Error")
        frappe.throw(_(f"Error deleting Lead: {str(e)}"))
...
def delete_all_leads():
    ...
    try:
        ...
        for lead in leads:
            ...
            deleted_count += 1
        frappe.db.commit()
        frappe.log_error(f"Deleted {deleted_count} leads", "IndiaMART Lead Deletion")
        frappe.msgprint(_(f"Successfully deleted {deleted_count} leads"))
        return {"success": True, "message": f"Deleted {deleted_count} leads"}

    except Exception as e:
        frappe.log_error(f"Error deleting leads: {str(e)}\n{frappe.get_traceback()}", "IndiaMART Lead Deletion Error")
        frappe.throw(_(f"Error deleting leads: {str(e)}"))
