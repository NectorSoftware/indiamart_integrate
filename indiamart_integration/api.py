...
#new code 
# apps/indiamart_integration/indiamart_integration/api.py
...
def add_lead(lead_data):
    ...
    try:
        ...
        # Parse query time and store in custom_querytime
        query_time = None
        if lead_data.get("QUERY_TIME"):
            try:
                query_time = datetime.strptime(lead_data["QUERY_TIME"], "%Y-%m-%d %H:%M:%S")
                ...
                frappe.log_error(f"Invalid QUERY_TIME format: {lead_data['QUERY_TIME']}", "IndiaMART Lead Creation Error")
                query_time = now_datetime()  # Fallback to current time if parsing fails
        ...
        doc = frappe.get_doc({
            ...
            "country": country_name,
            "status": lead_status,
            "creation": query_time or now_datetime(),  # Still set creation for ERPNext internal use
            "custom_querytime": query_time or now_datetime(),  # Store query time in custom field
            "modified": now_datetime(),
            "owner": frappe.session.user,
            "modified_by": frappe.session.user,
            "docstatus": 0,
...
def get_indiamart_leads(query_type=None, from_date=None, to_date=None):
    ...
    try:
        ...
        # Apply date filters using custom_querytime
        if from_date and to_date:
            try:
                from_date = getdate(from_date)
                to_date = getdate(to_date)
                filters["custom_querytime"] = ["between", [from_date, to_date]]
            except ValueError:
                frappe.log_error(f"Invalid date format: from_date={from_date}, to_date={to_date}", "Get IndiaMART Leads Warning")
                # Fallback to a default date range
                from_date = to_date = getdate(today())
        ...
        leads = frappe.get_all("Lead", 
            ...
            fields=[
                "name", "custom_unique_query_id", "custom_sender_name", 
                "custom_query_product_name", "custom_sender_address", "status", 
                "custom_querytime", "mobile_no", "email_id", "custom_query_message", "city",
                "custom_query_type"
            ]
        )
        ...
        frappe.log_error(f"Filtered leads for query_type '{query_type}': {len(leads)} leads", "IndiaMART Leads Fetch Debug")
        for lead in leads:
            frappe.log_error(f"Lead: {lead['name']}, Query Time: {lead['custom_querytime']}", "IndiaMART Leads Fetch Debug Detail")
